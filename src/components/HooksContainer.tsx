...
export const HooksContainer = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      process.env.NODE_ENV !== "test" &&
        warning(
          "HooksContainer is removed from DOM. its not supported, singleton hooks will stop updating."
        );
    };
  }, []);
  ...
  useEffect(() => {
    Store.mountIntoContainer = (item: any) =>
      setHooks((hooks: any) => [...hooks, item]);
...
export const addHook = (hook: any) => {
  if (
    !Store.hooksContainerRendered &&
    !Store.hooksContainerMountedAutomatically
  ) {
